...
package com.safasoft.pentaho.datatrans.dest.dao;
...
public class BaseDestDAO<T> {
  ...
  public T save(T domain) {
    sessionFactory2.getCurrentSession().saveOrUpdate(domain);
    return domain;
  }
  ...
  public T delete(T domain) {
    sessionFactory2.getCurrentSession().delete(domain);
    return domain;
  }
...
public T getById(int id) {
    return (T) sessionFactory2.getCurrentSession().get(domainClass, id);
  }
...
public List<T> getAll() {
    return sessionFactory2.getCurrentSession().createQuery("from " + domainClass.getName()).list();
  }
...
public List<T> getByRange(int start, int num) {
    return sessionFactory2.getCurrentSession().createQuery("from " + domainClass.getName())
   ...
   */
  public int count() {
    return ((Long) sessionFactory2.getCurrentSession().createQuery(
  ...
  public int truncateTable(String tableName) {
  	return (Integer) sessionFactory2.getCurrentSession().createSQLQuery(
  			"select truncate_table_func(:tableName)")
  			.setString("tableName", tableName.toLowerCase())
  			.uniqueResult();
  			/*return sessionFactory2.getCurrentSession().createSQLQuery(
